...
def fill_order(order, txes=[]):
    ...
    query = g.session.query(Order).filter(Order.tx_id != None, Order.filled == None, Order.buy_currency == order.sell_currency, Order.sell_currency == order.buy_currency, Order.sell_amount / Order.buy_amount >= order.buy_amount/order.sell_amount)   
    count = query.count()
    if count > 0:
